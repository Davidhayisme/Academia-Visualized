...
export default function PubsPer({ disease }: OastatusProps) {
  const [rdata, setrdata] = useState<{ name: string; value: number }[]>([]);

  useEffect(() => {
    if (!disease) return;

    const apiUrl = import.meta.env.VITE_API_URL + "/cpj";
    ...
    fetch(apiUrl, {
      ...
      .then((rawData) => {
        console.log(rawData)
        if (rawData && Array.isArray(rawData.cpj)) {
          setrdata(rawData.cpj);
        } else {
