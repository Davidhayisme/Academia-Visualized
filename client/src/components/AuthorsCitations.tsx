...
export default function ACitations({ disease }: OastatusProps) {
  ...
  useEffect(() => {
    ...
    const apiUrl = import.meta.env.VITE_API_URL + "/ac";
    ...
    fetch(apiUrl, {
      ...
      .then((rawData) => {
        console.log(rawData)
        if (rawData && Array.isArray(rawData.ac)) {
          setrdata(rawData.ac);
        } else {
  ...
  return (
    <Card className="data-card">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-medium">Citations by Popular Authors</CardTitle>
      </CardHeader>
      <CardContent>
